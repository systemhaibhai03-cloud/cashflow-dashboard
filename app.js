...
// Timeout wala fetch — taaki ek tab hang ho to poora dashboard na atke
...
async function fetchSheet(cfg){
  // timeout + ek retry, taaki hang na ho
  let res;
  try {
    res = await fetchWithTimeout(gvizUrl(cfg.name), 15000);
  } catch (e1) {
    try { res = await fetchWithTimeout(gvizUrl(cfg.name), 15000); }
    catch (e2) {
      const why = e2.name === 'AbortError' ? '15s mein jawab nahi (network hang/block)' : e2.message;
      throw new Error(`Tab "${cfg.name}" — ${why}`);
    }
  }
  const text = await res.text();
  const data = stripWrapper(text);
  // Google ne galat tab-naam ya aur koi error bheja?
  if (data.status === 'error'){
    const m = (data.errors && data.errors[0] && (data.errors[0].detailed_message || data.errors[0].message)) || 'unknown';
    throw new Error(`Tab "${cfg.name}" — Google error: ${m}. (Aksar tab ka naam exact match nahi karta.)`);
  }
